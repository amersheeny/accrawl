...
public class FlutterSecureStorage {
    ...
    private void initializeStorageCipher(NamespacedConfigSource configSource, SecurePreferencesCallback<Void> callback) {
        try {
            ...
            if (storageCipherFactory.requiresReEncryption()) {
                ...
                if (!hasNamespacedData(dataPreferences)) {
                    Log.i(TAG, "No encrypted data exists; replacing stale keys and algorithm metadata.");
            ...
            // Android 9–10 cannot use DEVICE_CREDENTIAL with a crypto-bound
            // BiometricPrompt. The caller must first complete Keyguard's
            // credential confirmation; successful Cipher initialization above
            // proves the short-duration Keystore authorization is active.
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R
                    && !config.isStrongBiometricOnly()) {
                storageCipher = storageCipherFactory.getCurrentStorageCipher(context, cipher);
                callback.onSuccess(null);
                return;
            }
    ...
    private void migrateData(NamespacedConfigSource configSource, SharedPreferences dataSource,
        ...
        try {
            ...
            String savedKeyAlg = storageCipherFactory.getSavedKeyAlgorithmName();
            String currentKeyAlg = storageCipherFactory.getCurrentKeyAlgorithmName();

            boolean fromBiometric = storageCipherFactory.isSavedKeyStoreBacked();
            boolean toBiometric = storageCipherFactory.isCurrentKeyStoreBacked();
    ...
    /**
     * Checks whether a preferences file contains data in the configured namespace.
     */
    private boolean hasNamespacedData(SharedPreferences preferences) {

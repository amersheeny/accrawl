...
public class StorageCipherFactoryTest {
    ...
    @Test
    public void keyStoreBackedDetection_usesKeyAlgorithmsNotStorageAlgorithms() {
        // Saved storage algorithm is AES_GCM while the saved KEY algorithm is RSA:
        // KeyStore-backed detection must read the key algorithm, not the storage one.
        saveAlgorithms("RSA_ECB_OAEPwithSHA_256andMGF1Padding", "AES_GCM_NoPadding");

        StorageCipherFactory f = factory("AES_GCM_NoPadding", "AES_GCM_NoPadding");

        assertFalse(f.isSavedKeyStoreBacked());
        assertTrue(f.isCurrentKeyStoreBacked());
        assertEquals("RSA_ECB_OAEPwithSHA_256andMGF1Padding", f.getSavedKeyAlgorithmName());
        assertEquals("AES_GCM_NoPadding", f.getCurrentKeyAlgorithmName());
    }

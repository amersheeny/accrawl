...
class FlutterSecureStorage {
  ...
  /// Releases cached Android cipher state so the next operation must
  /// authenticate again when user authentication is enforced.
  ...
  /// Deletes a local Android value without first decrypting it.
  ///
  /// Destruction must remain possible when authentication is unavailable.
  Future<void> deleteWithoutAuthentication({required String key}) async {
    if (defaultTargetPlatform != TargetPlatform.android) {
      await delete(key: key);
      return;
    }
    await _methodChannel.invokeMethod<void>('deleteWithoutAuthentication', {
      'key': key,
      'options': aOptions.toMap(),
    });

...
| `OPERATOR_TOKEN_TTL_HOURS` | Validity, in hours, of newly issued operator session tokens (optional; default `168` = 7 days; `1`–`720`). Keep the default when you sign in only from devices you control. Do not sign in from a shared or untrusted machine; if that is unavoidable, use `1` and revoke every session immediately afterwards. Shortening this limits how long a copied token can be reused — it does not make an untrusted machine safe, and tokens already issued keep their original expiry. An invalid or out-of-range value falls back to `168`. See [Security model](DEPLOY.md#security-model-what-protects-you) |
